...
struct VibeVoiceBatchCoreChecks {
    ...
    private static func checkWorkstationToolbarPolicy() throws {
        ...
        precondition(WorkstationSelection.queuedGeneration("queued").toolbarKind == .editor)
        ...
        precondition(WorkstationSelection.queuedGeneration("abc").id == "queued-abc")
    ...
    private static func checkStabilizationSourceAudit() throws {
        ...
        precondition(backendRuntimeServices.contains("internal func backendTrimmedOutputLines"))
        precondition(backendRuntimeServices.contains("internal func backendDockerTableRows"))
        ...
        precondition(healthSetup.contains("backendDockerTableRows(imageResult.combinedOutput)"))
        precondition(healthSetup.contains("backendTrimmedNonEmpty(profile.containerName)"))
        precondition(!healthSetup.contains("private func dockerOutputRows"))
        precondition(operationExecutor.contains("backendTrimmedOutputLines(list.combinedOutput)"))
        precondition(operationExecutor.contains("backendTrimmedOutputLines(result.combinedOutput)"))
        precondition(operationExecutor.contains("backendTrimmedNonEmpty(profile.containerName)"))
    ...
    private static func checkPostRefactorUXPolish() throws {
        ...
        let generationList = try String(contentsOf: views.appendingPathComponent("GenerationListView.swift"), encoding: .utf8)
        ...
        let sessionDetail = try String(contentsOf: views.appendingPathComponent("SessionDetailView.swift"), encoding: .utf8)
        let largeLogTextView = try String(contentsOf: views.appendingPathComponent("LargeLogTextView.swift"), encoding: .utf8)
        let largePlainTextView = try String(contentsOf: views.appendingPathComponent("LargePlainTextView.swift"), encoding: .utf8)
        ...
        let telemetry = try String(contentsOf: support.appendingPathComponent("FonimTelemetry.swift"), encoding: .utf8)
        ...
        let queuedGenerationItem = try String(contentsOf: root.appendingPathComponent("Sources/VibeVoiceBatch/Models/QueuedGenerationItem.swift"), encoding: .utf8)
        ...
        precondition(!generationWorkspace.contains("prewarmedSessionDetail"))
        precondition(!generationWorkspace.contains("showsNavigationTitle: false"))
        ...
        precondition(sidebar.contains("case .queuedGeneration:"))
        ...
        precondition(sidebar.contains("SidebarFooterMetric(label: \"Queued\""))
        precondition(sidebar.contains("SidebarFooterMetric(label: \"Generations\""))
        precondition(generationList.contains("struct GenerationListView"))
        precondition(generationList.contains("GenerationListDisplayModel"))
        precondition(generationList.contains("ForEach(model.items)"))
        precondition(!generationList.contains("Section(\"Queued\")"))
        precondition(!generationList.contains("Section(\"History\")"))
        precondition(generationList.contains("GenerationListItem"))
        precondition(generationList.contains("ScriptSectionCapsule"))
        ...
        precondition(generationList.contains("return lhs.position > rhs.position"))
        precondition(generationList.contains("sessionIDByBatchItemID"))
        precondition(generationList.contains("hiddenSessionIDs"))
        precondition(generationList.contains("isLikelySameActiveGeneration"))
        precondition(generationList.contains("session.metadata.status == .running"))
        precondition(generationList.contains("session.inputText.trimmedOrNil == queued.sourceText.trimmedOrNil"))
        precondition(generationList.contains("abs(session.metadata.createdAt.timeIntervalSince(queueStart)) < 90"))
        precondition(generationList.contains("visibleSessions"))
        precondition(generationList.contains("shouldPreferBatchSectionOrder"))
        precondition(generationList.contains("abs(lhsDate.timeIntervalSince(rhsDate)) < 1"))
        precondition(generationList.contains("selection = .queuedGeneration(item.id)"))
        precondition(generationList.contains("FonimTelemetry.selectedGeneration(kind: \"queued\""))
        precondition(generationList.contains("FonimTelemetry.selectedGeneration(kind: \"history\""))
        precondition(!generationList.contains("store.selectedQueueItemID = item.id"))
        precondition(!generationList.contains("store.selectedSessionID = record.id"))
        precondition(!generationList.contains("store.selectedSessionID = sessionID"))
        ...
        precondition(!content.contains("syncStoreSelectionAfterPaint"))
        precondition(!content.contains("await Task.yield()"))
        precondition(content.contains("selectedToolbarSession"))
        precondition(!content.contains("store.selectedSession"))
        precondition(inspector.contains("let selectedSession: SessionRecord?"))
        precondition(!inspector.contains("store.selectedSession"))
        precondition(sessionDetail.contains(".task(id: logLoadTaskID)"))
        precondition(sessionDetail.contains("LargePlainTextView("))
        precondition(sessionDetail.contains("telemetryKind: \"session-input\""))
        precondition(sessionDetail.contains("telemetryKind: \"session-metadata\""))
        precondition(sessionDetail.contains("FonimTelemetry.sessionLogLoadStarted"))
        precondition(sessionDetail.contains("FonimTelemetry.sessionLogLoadFinished"))
        precondition(sessionDetail.contains("scrollToBottomTrigger: \"\\(record.id)-\\(selectedPane.rawValue)-\\(loadedLogSessionID ?? \"\")\""))
        precondition(generationWorkspace.contains("telemetryKind: \"queued-input\""))
        precondition(generationWorkspace.contains("FonimTelemetry.detailAppeared(kind: \"queued\""))
        precondition(generationWorkspace.contains("FonimTelemetry.detailAppeared(kind: \"history\""))
        precondition(largePlainTextView.contains("struct LargePlainTextView: NSViewRepresentable"))
        precondition(largePlainTextView.contains("NSTextView()"))
        precondition(largePlainTextView.contains("isEditable = false"))
        precondition(largePlainTextView.contains("isSelectable = true"))
        precondition(telemetry.contains("import OSLog"))
        precondition(telemetry.contains("import os.signpost"))
        precondition(telemetry.contains("category: \"GenerationSelection\""))
        precondition(telemetry.contains("os_signpost(.event"))
        precondition(!sessionDetail.contains("showsNavigationTitle"))
        precondition(sessionDetail.contains("Task.detached(priority: .utility)"))
        precondition(sessionDetail.contains("store.cachedLogText(for: record)"))
        precondition(!sessionDetail.contains("store.logText(for: record)"))
        precondition(!content.contains(".keyboardShortcut(\" \", modifiers: [])"))
        precondition(largeLogTextView.contains("postsBoundsChangedNotifications = true"))
        precondition(largeLogTextView.contains("isFollowingTail"))
        precondition(largeLogTextView.contains("scrollToBottomTrigger"))
        precondition(largeLogTextView.contains("didChangeTrigger"))
        precondition(largeLogTextView.contains("boundsDidChangeNotification"))
        precondition(largeLogTextView.contains("scrollToBottom()"))
        precondition(largeLogTextView.contains("distanceFromBottom < 28"))
        ...
        precondition(inspector.contains("case .queuedGeneration(let itemID):"))
        precondition(inspector.contains("queuedGenerationRecipeSection(itemID)"))
        precondition(inspector.contains("This queued generation is already committed"))
        precondition(inspector.contains("private var generationPresetSection"))
        precondition(inspector.contains("Picker(\"Preset\", selection: generationPresetBinding)"))
        precondition(inspector.contains("applySelectedGenerationPreset()"))
        precondition(inspector.contains("saveCurrentGenerationRecipe()"))
        precondition(inspector.contains("store.applyGenerationPreset(preset)"))
        precondition(queuedGenerationItem.contains("let backendID: String"))
        precondition(queuedGenerationItem.contains("let modelID: String"))
        precondition(queuedGenerationItem.contains("backendID = job.backendID"))
        precondition(queuedGenerationItem.contains("modelID = job.modelID"))
        ...
        precondition(inspector.contains("SessionRecipeSnapshot(session: session)"))
        precondition(inspector.contains("matchingSessionGenerationPreset(recipe)"))
        precondition(inspector.contains("Label(\"Duplicate as New\", systemImage: \"doc.on.doc\")"))
        precondition(inspector.contains("Label(\"Save Recipe\", systemImage: \"plus\")"))
        precondition(inspector.contains("saveSessionGenerationRecipe(recipe)"))
        precondition(inspector.contains("struct SessionRecipeSnapshot"))
        precondition(inspector.contains("inferBackendID(from session: SessionRecord)"))
        precondition(inspector.contains("inferModelID(from session: SessionRecord, backendID: String)"))
        precondition(inspector.contains("contains(\"realtime_model_inference_from_file\")"))
        precondition(inspector.contains("return BackendProfiles.kokoroTTS.requiredModels.first?.id"))
        ...
        precondition(appStore.contains("selectQueuedItem: Bool = true"))
        precondition(appStore.contains("if selectQueuedItem"))
        precondition(appStoreScriptQueue.contains("var firstQueuedJobID: String?"))
        precondition(appStoreScriptQueue.contains("selectQueuedItem: false"))
        precondition(appStoreScriptQueue.contains("selectedQueueItemID = firstQueuedJobID"))
        precondition(appStore.contains("func requestSelection(_ selection: WorkstationSelection?)"))
        precondition(appStoreScriptQueue.contains("requestSelection(.section(.batches))"))
