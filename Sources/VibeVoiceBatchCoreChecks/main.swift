...
struct VibeVoiceBatchCoreChecks {
    ...
    private static func checkPostRefactorUXPolish() throws {
        ...
        precondition(generationList.contains("ForEach(orderedSessions)"))
        precondition(generationList.contains("return $0.metadata.createdAt < $1.metadata.createdAt"))
        precondition(generationList.contains("proxy.scrollTo(sessionID, anchor: .bottom)"))
